...
def record_paper_candidates(live_run:dict[str,Any],path:str|Path)->dict[str,Any]:
    target=Path(path);seen=_existing(target);added=0
    for game in live_run.get("games") or []:
        ...
        for c in (game.get("decision") or {}).get("candidates") or []:
            ...
            row={"entry_key":key,"entry_type":"RESEARCH_PAPER_CANDIDATE","model_generation":MODEL_GENERATION,"probability_policy_id":PROBABILITY_POLICY_ID,"game_id":game.get("game_id"),"game_date":game.get("game_date"),"commence_time":game.get("commence_time"),"home":game.get("home"),"away":game.get("away"),"phase":game.get("phase"),"market":c.get("market"),"selection":c.get("selection"),"price":c.get("price"),"line":c.get("line"),"model_probability":c.get("model_probability"),"lower_probability":c.get("lower_probability"),"sharp_probability_entry":c.get("sharp_probability"),"execution_book":c.get("execution_book"),"entry_at":game.get("analyzed_at"),"status":"PAPER"}
