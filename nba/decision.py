...
def evaluate_candidate(*, selection: str, market: str, model_probability: float, lower_probability: float, price: float, sharp_probability: float | None, certified: bool, market_fresh: bool, lineup_unresolved: bool = False) -> dict[str, Any]:
    breakeven = 1.0 / float(price)
    model_edge = 100.0 * (model_probability - breakeven)
    robust_edge = 100.0 * (lower_probability - breakeven)
    sharp_edge = None if sharp_probability is None else 100.0 * (model_probability - sharp_probability)
    robust_sharp_edge = None if sharp_probability is None else 100.0 * (lower_probability - sharp_probability)
    failures: list[str] = []
    ...
    return {
        "selection": selection,
        "market": market,
        "status": "BET" if not failures else "NO_BET",
        "model_probability": model_probability,
        "lower_probability": lower_probability,
        "price": price,
        "breakeven_probability": breakeven,
        "model_edge_pp": model_edge,
        "robust_edge_pp": robust_edge,
        "sharp_probability": sharp_probability,
        "sharp_edge_pp": sharp_edge,
        "robust_sharp_edge_pp": robust_sharp_edge,
        "failures": failures,
    }
