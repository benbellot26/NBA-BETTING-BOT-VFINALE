...
def analyze_game(*, home: TeamMetrics, away: TeamMetrics, context: GameContext, spread_line: float, total_line: float, books_by_market: dict[str,list[dict[str,Any]]] | None = None, certification: dict[str,Any] | None = None, home_rotation: Iterable[RotationPlayer] | None = None, away_rotation: Iterable[RotationPlayer] | None = None, market_fresh: bool = True) -> dict[str,Any]:
    ...
    probs=asdict(surface)
    unresolved=projection.data_quality=="LINEUP_UNCERTAIN"
    bands=intervals(probs, data_quality=projection.data_quality, market_fresh=market_fresh, unresolved_key_player=unresolved)
    ...
    for key,market,left,right,point in specs:
        ...
        sharp_p=None if not sharp else sharp.get(left)
        mcert=((cert.get("markets") or {}).get(market) or {}).get("betting_certified") is True and cert.get("certified") is True
        candidate=evaluate_candidate(selection=key,market=market,model_probability=float(probs[key]),lower_probability=float(bands["selections"][key]["lower"]),price=float(execution["price"]),sharp_probability=sharp_p,certified=mcert,market_fresh=market_fresh,lineup_unresolved=unresolved)
        candidate["execution_book"]=execution.get("bookmaker"); candidate["game_id"]=context.game_id; candidate["line"]=point
